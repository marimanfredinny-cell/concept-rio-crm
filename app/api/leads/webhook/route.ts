...
export async function POST(req: NextRequest) {
  ...
  // Extração dos campos — aceita variações de nome
  const nome = (data.nome ?? data.name ?? data.nome_completo ?? '').trim()
  const telefone = (data.telefone ?? data.phone ?? data.celular ?? data.whatsapp ?? '').trim()
  const email = (data.email ?? '').trim() || undefined
  const mensagem = (data.mensagem ?? data.message ?? data.descricao ?? '').trim() || undefined
  const interesse = (data.imovel ?? data.imovel_titulo ?? data.property ?? data.produto ?? '').trim() || undefined
  const bairro = (data.bairro ?? data.bairro_interesse ?? data.neighborhood ?? '').trim() || undefined
  ...
  if (!nome || !telefone) {
    return NextResponse.json(
      { error: 'Campos obrigatórios ausentes: nome e telefone' },
      { status: 400, headers: CORS }
    )
  }

  // Detectar origem pelo UTM
  function resolveOrigem(): string {
    const src = (utmSource ?? '').toLowerCase()
    const med = (utmMedium ?? '').toLowerCase()
    if (src.includes('google') || med.includes('cpc')) return 'Google Ads'
    if (src.includes('facebook') || src.includes('instagram') || src.includes('meta') || med.includes('paid_social')) return 'Meta Ads'
